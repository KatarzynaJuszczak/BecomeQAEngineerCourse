from src.applications.github.api.github_api import test_api_logger, config


def test_search_for_existing_user(github_api_app):
    test_api_logger.info("------- test_search_for_existing_user -------")

    existing_user_name = config.VALID_DATA["USER_NAME"]
    users = github_api_app.search_user(existing_user_name)

    test_api_logger.info(f"Total count is {users['total_count']}")

    assert users["total_count"] != 0


def test_search_for_non_existing_user(github_api_app):
    test_api_logger.info("------- test_search_for_non_existing_user -------")
